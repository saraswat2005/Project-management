...
export const Sidebar = () => {
    ...
    const disappearOrKeep = (location) => {
        const forbiddens = ["/login", "/signup"]
        let value = false
        for (let i = 0; i < forbiddens.length; i++) {
            if (forbiddens[i] === location) {
                console.log(forbiddens[i], location)
                value = true
                break
            }
        }
    ...
    console.log("check ", disappearOrKeep("/login"));

    useEffect(() => {
        console.log(location);
        setDisplayObj(disappearOrKeep(location.pathname))
        console.log(displayObj)
    }, [location])

    return (
        <nav
            className="bg-white shadow-lg h-screen fixed top-0 left-0 min-w-[250px] py-6 px-4 font-[sans-serif] overflow-auto"
        >
            <Link to="/">
                <h1 className={"text-medium"} style={{"font-size": "1.5rem"}}>PlanSync</h1>
            </Link>
