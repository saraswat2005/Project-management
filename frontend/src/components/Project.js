...
export const Project = (props) => {
    ...
    const showTask = (projectID) => {
        const url = '/projects/'+ projectID+'/alltasks'
        navigate(url)
    }
    return (
        <>
            <tr className="hover:bg-gray-50" style={{position: "relative", left: "3vw"}}>
                ...
                <td className="p-4 text-[15px] text-gray-800">
                    {props.project.manager ? props.manager : "Not Assigned"}
                </td>
                <td className="p-4 text-[15px] text-gray-800">
                    {props.project.createdOn.slice(0, 10)}
                </td>
                <td className="p-4">
                    ...
                    <div>
                        <button type="button" onClick={() => {showTask(props.project._id)}}
                            className="w-10 h-10 inline-flex items-center justify-center rounded-lg border-none outline-none bg-blue-600 hover:bg-blue-700 active:bg-blue-600">
